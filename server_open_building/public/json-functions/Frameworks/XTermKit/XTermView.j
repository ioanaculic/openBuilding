...
@implementation XTermView : CPWebView {
...
- (void) akInitialize {
	...
	[self setMainFrameURL:[[CPBundle bundleForClass:[self class]] pathForResource:"xterm.html"]];
...
- (void) webView:(CPWebView)aWebView didFinishLoadForFrame:(id)aFrame {
	...
	[self setXterm:[self objectByEvaluatingJavaScriptFromString:@"XTermInit()"]];
...
- (BOOL) acceptsFirstResponder {
	...
	return (!![self xterm]); // && [self isEditable] && [self isEnabled]);
...
// @implementation ACEEditorView (Wyliodrin)

// //	I am not sure if this is adequare, because actually a lot of state is delegated to ACE itself; we are not exposing an undo manager or anything like that
// //	Actually this part does not work with Objective-J / Cappuccino”s global undo / redo at all

// - (void) open:(CPString)filename {

// 	console.log ([self editor]);
// 	if ([self editor])
// 	{
// 		[self objectByEvaluatingJavaScriptFromString:@"ACEShareOpen('"+filename+"');"];
// 	}
// }
